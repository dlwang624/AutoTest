		...
		<div class="main">
			...
			<div class="main-content" style="padding:20px 10px 0px 10px;">
				<div class="container-fluid">
					<div class="fl"><h3 class="page-title">已有测试用例</h3></div>
					<div class="fr">
						<button id="batch" class="btn btn-default">批量执行</button>
					</div>
					<div class="clearfix"></div>
					<div class="h30 mb10">
						<div class="fl 30">
							<div class="fl h30"></div>
							<div class="fl h30">
								<form id="changeSelect" action="CMS/changeForm.htm" method="post">
									<input id="proid" name="proid" type="text" value="${projectid}" class="none"/>
									<select id="filetab" class="form-control input-sm">
										...
										<table class="table">
											...
											<tbody>
											    ...
											    <td select="0" style="padding:4px;">
											    	<div>
												    	<div class="fl">
												    		<c:if test="${test.test.flag == 0}"><span title="${ test.test.id}" class="fa fa-exclamation-circle yellow"></span></c:if>
												    		<c:if test="${test.test.flag == 1}"><span title="${ test.test.id}" class="fa fa-minus-circle red"></span></c:if>
												    		<c:if test="${test.test.flag == 2}"><span title="${ test.test.id}" class="fa fa-check-circle-o green"></span></c:if>
												    	</div>
												    	<div class="fl ellipsis" style="width:250px;text-align:left;cursor: pointer;" title="${test.testname }" onclick="give(this,'${test.testname }',<c:if test="${ test.test.id == null || test.test.id == ''}">null</c:if><c:if test="${ test.test.id != null || test.test.id != ''}">${test.test.id }</c:if>)">
												    		${test.testname }
												    	</div>
												    	<div class="clearfix"></div>
											    	</div>
											    ...
											    <td style="padding:4px;">
											    	<div class="clearfix">
											    		<c:if test = "${!test.flag}">
															<span class="fa fa-refresh cursor ml2" title="同步" onclick="selTestOrSyncQC(this,'${test.testname}','${test.test.id}')"></span>
														</c:if>
											    		
											    		<c:if test = "${test.flag}">
															<span class="fa fa-pencil-square-o cursor ml2" title="查看" onclick="selTestOrSyncQC(this,'${test.testname}','${test.test.id}')"></span>
														</c:if>
											    		<span class="fa fa-expeditedssl cursor ml2" title="解锁" onclick="unlockTest('${test.test.id}')"></span>
											    		<span class="fa fa-search cursor ml2" title="格式check" onclick="checkQC('${test.testname}')"></span>
											    	</div>
											    </td> 
											</tr> 
											</c:forEach>
