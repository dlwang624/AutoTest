...
function selTestOrSyncQC(btn,testName,id){
	...
	if(btnVal=="查看"){
			...
			$.ajax({
			  ...
			  success: function(data){
				if(data.value=="1"){
					...
					window.location.href="CMS/SelTest.htm?id="+id+"&&proid="+projectid+"&&viewname=testlist";
					$(btn).removeAttr("disabled");
				}
			  },
			  error: function (xhr, status, text) {
		...
		disableMethod(btn,"selTestOrSyncQC");
		//同步
		$.ajax({
		  type: 'POST',
		  ...
		  dataType: "json",
		  success: function(data){
			  btn.selTestOrSyncQC.recover();
			  if(data.msg=="null"){
				  layer.alert("此用例还没添加步骤");
				  $(btn).removeAttr("disabled");
			  }else{
				  layer.alert(data.msg);
				  $(btn).attr("title","查看");
				  $(btn).attr("class","fa fa-pencil-square-o cursor ml2");
				  $(btn).unbind("click");
				  $(btn).click(function(){
