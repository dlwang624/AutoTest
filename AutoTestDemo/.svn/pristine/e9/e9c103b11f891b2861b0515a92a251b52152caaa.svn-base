					...
					<div class="clearfix">
						...
						<div class="fl" style="margin-left:20px;">
							<select id="switchAmbient" class="form-control input-sm">
								...
								<c:if test="${ null != swlist}">
									<option value="-1" <c:if test="${swid == -1 }">selected="selected"</c:if>>请选择执行环境</option>
								</c:if>
								<c:forEach items="${ swlist}" var="sw">
									<option value="${sw.id }" <c:if test="${swid == sw.id }">selected="selected"</c:if>>${sw.amdesc }</option>
								</c:forEach>
							</select>
						</div>
										...
										<div class="contentDiv">
											...
											<div class="fr" style="width:50%;">
												...
												<div class="mt10">
													...
													<div class="fl">
														<input type="text" value="${ time}" id="timetemp" class="none">
